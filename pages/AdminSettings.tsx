...
const AdminSettings: React.FC<AdminSettingsProps> = ({ config, onSave }) => {
  ...
  const testConnection = async () => {
    ...
    try {
      // 发送一个最简单的测试请求
      const result = await geminiService.callAI("Connectivity test. Reply exactly with 'OK'.", formData, "You are a connectivity tester.");
      
      if (result.includes("OK") || (result.length > 0 && !result.includes("[系统") && !result.includes("[鉴权") && !result.includes("[接口"))) {
      ...
      <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100 flex gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-blue-700 mt-1 leading-relaxed">
            1. 为了确保 AI 研判的准确性，推荐使用 <b>google/gemini-3-flash-preview</b> 模型。<br/>
            2. 如果测试连接失败，请确认 API Key 是否已激活，且 OpenRouter 账户内有足够余额。<br/>
            3. 配置成功后，系统所有 AI 模块将立即启用该设置。
          </p>
        </div>
      </div>
    </div>
