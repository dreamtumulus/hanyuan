...
import { PersonalInfo, ExamReport, PsychTestReport } from '../types';
...
const PsychCounselingPage: React.FC<PsychCounselingPageProps> = ({ officerInfo, exams, psychReports }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: newMsgs.map(m => ({ parts: [{ text: m.text }], role: m.role === 'user' ? 'user' : 'model' })) as any,
      });
